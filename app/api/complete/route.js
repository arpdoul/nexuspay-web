...
export async function POST(req){
  ...
  try{
    ...
    await db.from('payments').insert({from_agent:'Orchestrator',to_agent:agent.name,to_address:agent.address,amount:task.reward,tx_id:txId,status:'CONFIRMED',task_id});
    return Response.json({success:true,txId,amount:task.reward,to:agent.address});
  }catch(e){
